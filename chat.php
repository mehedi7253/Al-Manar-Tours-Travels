    ...
    if (not_logged_in() === true)
    {
        ...
        if (isset($_POST['btn_msg']))
        {
            ...
            if ($msg && $image && $m_id && $m_name)
            {
                ...
                $chat = "INSERT INTO chating (message, image, m_id, m_name, date, status) VALUES ('$msg', '$image', '$m_id', '$m_name', '$created', '0')"; // send message to admin
                $res_chat = mysqli_query($connect, $chat); // connect with query and database
            }
        }
    }
    ?>
        ...
        <div class="card-body"  style="margin: 5px; padding: 5px; height: 300px; overflow: auto;">
            ...
            if (not_logged_in() === true)
            {
                ...
                if (isset($_SESSION['id'])){
                    ...
                    $view_message = "SELECT * FROM chating WHERE m_id = $id ORDER BY date ASC"; //view all message based on user id
                    $res_msg = mysqli_query($connect, $view_message); // connect with query and database
