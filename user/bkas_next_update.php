            ...
            <div class="col-md-12 col-sm-12 mt-5 mb-5">
                ...
                <div class="col-md-8 col-sm-12 float-left">
                    <div class="card" style="background-color: #E3106D">
                        <div class="card-header">
                            <h2 class="text-white">Pay With Bkas</h2>
                            <?php
                            if (isset($_GET['pay'])){
                                $id = $_GET['pay'];

                                $sql = "SELECT * FROM booking, invoice WHERE invoice.booking_id = $id"; // payment data
                                $res = mysqli_query($connect, $sql); // connect with query and database

                                $customer_data = mysqli_fetch_assoc($res);

                                $amount = $customer_data['price'] - $customer_data['pay_amount']; //view due
                                $due_amount = $customer_data['pay_amount']; // duew amount
                                $total_due = $due_amount + $amount; // total due amount

                                if (isset($_POST['pay']))
                                {
                                    $pay_amount = $_POST['pay_amount'];
                                    $sql_up = "UPDATE invoice SET pay_amount = '$pay_amount', payment_by = 'Bkash' WHERE booking_id = $id"; // payment
                                ...
                                {
                                    $pay_amount = $_POST['pay_amount']; // cancel payment

                                    $sql_cancel    = "UPDATE invoice SET pay_amount = '$customer_data[pay_amount]' WHERE booking_id = $id"; // cancel payment
                                    $res_cancel    = mysqli_query($connect, $sql_cancel); // connect with query and database
                                ...
                                <div class="col-md-8 mx-auto">
                                    <form action="" method="post">
                                        <div class="form-group">
                                            ...
                                            <input type="text"  name="pay_amount"  class="form-control" value="<?php echo $amount;?>">
                                        ...
                                        <div class="form-group">
                                            <input type="submit" class="btn col-md-5 p-1" value="Submit" name="pay" style="background-color: #B6195E; color: white">
