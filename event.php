...
<section class="main_content">
    <div class="container">
        <div class="row">
            <?php
                $sql_get_event = "SELECT * FROM event"; //fetch all data from event table
                ...
                while ($row = mysqli_fetch_assoc($res_event)){?>
                    <div class="col-md-6 col-sm-12 float-left mt-3">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <h3 class="font-weight-bold text-capitalize mt-3 text-center"><?php echo $row['title'];?></h3>
                                <hr style="border-bottom: 1px solid silver"/>
                                <label class="font-weight-bold">Start Date: <?php echo date('M-D-Y', strtotime($row['date']))?></label><br/>
                                <label class="font-weight-bold">Start Time: <?php echo date('g:i A', strtotime($row['time']))?></label><br/>
                                <p><?php echo $row['application'];?></p>
                            </div>
                        </div>
                    </div>
                <?php }?>
