    ...
    <div id="content-wrapper">
        ...
        <div class="container-fluid">
            ...
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Add Multi Menu</li>
            </ol>
                ...
                <div class="col-md-10 mx-auto mt-4 mb-5">
                    <div class="card">
                        <div class="card-header">
                            <h3>Add Multi Menu <span class="float-right"><a href="manage-multi.php" class="btn btn-primary">All Multi Menu</a></span></h3>
                        </div>
                        <div class="card-body">
                            <h5>
                                <?php
                                if (isset($_POST['btn']))
                                {
                                    ...
                                    $multi_menu_name = $_POST['multi_menu_name'];
                                    $description     = $_POST['description'];

                                    // check validatioon
                                    if ($multi_menu_name == ""){
                                        $_SESSION['menu_name'] = 'Please Enter Multi Menu Name';
                                    }
                                    if ($description == ""){
                                        $_SESSION['desc'] = 'Please Enter Description';
                                    }

                                    if ($sub_menu && $multi_menu_name && $description)
                                    {
                                        $create = @date('Y-m-d H:i:s');

                                        $add_submenu = "INSERT INTO multi_menu (sub_menu_id, multi_menu_name, status, description, date) VALUES ('$sub_menu', '$multi_menu_name', '0', '$description', '$create')"; //
                                        $result      = mysqli_query($connect, $add_submenu); // connect with query and database

                                        $_SESSION['success'] = 'New Multi Menu Added successfully';
//                                        echo "<script>document.location.href='add-slider.php'</script>";
                                    }else{
                                        $_SESSION['error'] = 'Adding New Multi Menu Failed';
//                                        echo "<script>document.location.href='add-slider.php'</script>";
                            ...
                            <form action="" method="post">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="sub_menu_id">
                                        <?php
                                            $sub_menu = "SELECT * FROM submenu"; //select query
                                            $res      = mysqli_query($connect, $sub_menu); // connect with query and database

                                            while ($sub = mysqli_fetch_assoc($res)){?>
                                            <option value="<?php echo $sub['id'];?>"><?php echo $sub['menu_name'];?></option>
                                        <?php }
                                        ?>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label>Multi Menu Name: </label>
                                    <input type="text" name="multi_menu_name" class="form-control" placeholder="Enter Menu Name">
                                </div>
                                <div class="form-group">
                                    <label>Multi Menu Description: </label>
                                    <textarea name="description" class="form-control" placeholder="Enter Description"></textarea>
                                </div>
                                <div class="form-group">
                                    <input type="submit" name="btn" class="btn btn-success" value="Add New Multi Menu">
                                </div>
                            </form>
                        </div>
                    </div>
