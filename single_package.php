                ...
                <div class="row">
                    <div class="col-md-12 col-sm-12">
                        ...
                        if (isset($_GET['single']))
                        {
                            ...
                            $sql = "SELECT submenu.id, submenu.main_menu,  submenu.menu_name, packages.pakage_id, packages.menu_id, packages.pakage_type_id, packages.member_id, 
                                    packages.image, packages.duration, packages.application AS pak_desc, package_price.id, 
                                    package_price.price, package_price.package_name, team_member.id, team_member.name,team_member.phone,
                                    team_member.position, team_member.application AS Team 
                                    FROM packages, package_price, team_member, submenu
                                    WHERE packages.pakage_type_id = package_price.id 
                                    AND packages.member_id = team_member.id 
                                    AND packages.menu_id = submenu.id
                                    AND packages.pakage_id = '$id'";
//                            $sql = "SELECT * FROM packages, package_price, team_member WHERE  packages.pakage_type_id = package_price.id AND packages.member_id = team_member.id AND  team_member.application AS con AND pakage_id = $id"; // get all package by id
                            $res = mysqli_query($connect, $sql); // connect with query and database

                            $data = mysqli_fetch_assoc($res);
                        }
                        ?>
                        <h3 class="mt-5 font-weight-bold"><?php echo $data['package_name'];?></h3>
                        ...
                        <div class="col-md-7 col-sm-12 float-left">
                            <div class="card">
                                <div class="card-header" style="background-color: #F7F7F7">
                                    <h3 class="text-center font-weight-bold" style="color: #0983F7"><?php echo $data['package_name'];?></h3>
                                </div>
                                <div class="card-body" style="background-color: #F7F7F7">
                                    <label class="font-weight-bold">Package Duration: <span class="font-weight-normal text-capitalize"><?php echo $data['duration'];?></span></label><br/>
                                    <label class="font-weight-bold mt-4">Package Description: <span class="font-weight-normal text-capitalize"><?php echo $data['pak_desc'];?></span></label><br/>
                                    <label class="font-weight-bold mt-4">Package Price: <span class="font-weight-bold text-capitalize text-primary "><?php echo number_format($data['price'], 2);?> Taka (Per Person)</span></label><br/>



                                    <?php
                                        if ($data['main_menu'] == 'umrah_offers'){

                                        }else{
                                            echo "  <label class=\"font-weight-bold mt-4\">Pre-Registration Info: <span class=\"font-weight-normal text-capitalize\">
                                    ...
                                    <label class="text-justify ml-3"><?php echo $data['Team'];?></label>
                                </div>
